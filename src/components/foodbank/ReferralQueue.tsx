...
export default function ReferralQueue() {
  ...
  useEffect(() => {
    // Listen for vouchers actively being handled by volunteers
    ...
    const unsubscribe = onSnapshot(vouchersQuery,
      ...
      (err) => {
        ...
        setError("Failed to open real-time connection to foodbank database.");
        setLoading(false);
      }
    );
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center p-12 text-slate-500">
        ...
        <p className="text-sm font-medium">Connecting to live voucher queue...</p>
      </div>
    );
  }
    ...
    <div className="w-full max-w-6xl mx-auto p-4">
      ...
      <div className="bg-white border border-slate-200 border-t-0 rounded-b-xl shadow-sm p-6">
        <div className="sm:flex sm:items-center sm:justify-between border-b border-slate-100 pb-5 mb-6">
          <div>
            <h2 className="text-xl font-bold text-slate-900 tracking-tight">Referral & Packing Queue</h2>
            <p className="text-xs font-medium text-slate-500 mt-1 uppercase tracking-wider">Active Operations Desk</p>
          </div>
        ...
        {vouchers.length === 0 ? (
          <div className="text-center py-16 border-2 border-dashed border-slate-100 rounded-xl bg-slate-50/50">
            ...
            <p className="mt-1 text-xs text-slate-500">No pending client intakes or manifests requiring collection.</p>
          </div>
        ) : (
                    ...
                    <div className="flex justify-between items-start mb-4">
                      <div>
                        <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">
                          {voucher.id.startsWith('voucher-') ? 'Regional Agency Issued' : 'Verified Partner Voucher'}
                        </p>
                        <h4 className="text-base font-bold text-slate-900 mt-0.5 tracking-tight">
                          Ref: {voucher.client_name ? voucher.client_name : `Client [${voucher.id.slice(0, 8)}]`}
                        </h4>
                      </div>
                      ...
                      <div className="space-y-4">
                        <div className="bg-slate-50 rounded-xl p-3.5 border border-slate-100">
                          <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-2.5 flex items-center">
                            ...
                            Manifest Allocation Requirements
                          </p>
